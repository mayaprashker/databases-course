INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 318  -- Shawshank Redemption, The (1994)
, 9
, 'hai bramli'
, 'Notable for screenplay and dialogue'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 10
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'A great double feature for drama lovers'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 898  -- Philadelphia Story, The (1940)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 475  -- In the Name of the Father (1993)
, 10
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1204  -- Lawrence of Arabia (1962)
, 10
, 'hai bramli'
, 'Notable for screenplay and dialogue'
, 'Their critical reception and audience overlap is strong'
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1235  -- Harold and Maude (1971)
, 9
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 2959  -- Fight Club (1999)
, 9
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Actors deliver standout roles in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1276  -- Cool Hand Luke (1967)
, 9
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1221  -- Godfather: Part II, The (1974)
, 10
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 48516  -- Departed, The (2006)
, 10
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1213  -- Goodfellas (1990)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 912  -- Casablanca (1942)
, 10
, 'hai bramli'
, 'Share narrative style or pacing'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 58559  -- Dark Knight, The (2008)
, 9
, 'hai bramli'
, 'Share narrative style or pacing'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 926  -- All About Eve (1950)
, 9
, 'hai bramli'
, 'Share narrative style or pacing'
, 'Their critical reception and audience overlap is strong'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1245  -- Miller''s Crossing (1990)
, 10
, 'hai bramli'
, 'Both movies are award-winning'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 527  -- Schindler''s List (1993)
, 10
, 'hai bramli'
, 'Both movies are award-winning'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 3275  -- Boondock Saints, The (2000)
, 10
, 'hai bramli'
, 'Both movies are award-winning'
, 'Their critical reception and audience overlap is strong'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1208  -- Apocalypse Now (1979)
, 10
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'Narrative tension and payoff are comparable'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 2329  -- American History X (1998)
, 10
, 'hai bramli'
, 'Popular within the same genre community'
, 'Their critical reception and audience overlap is strong'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1233  -- Boot, Das (Boat, The) (1981)
, 9
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1104  -- Streetcar Named Desire, A (1951)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 9
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'A great double feature for drama lovers'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 898  -- Philadelphia Story, The (1940)
, 10
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 475  -- In the Name of the Father (1993)
, 9
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1204  -- Lawrence of Arabia (1962)
, 9
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Actors deliver standout roles in both'
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1235  -- Harold and Maude (1971)
, 9
, 'hai bramli'
, 'Popular within the same genre community'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 2959  -- Fight Club (1999)
, 9
, 'hai bramli'
, 'Both movies are award-winning'
, 'Narrative tension and payoff are comparable'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1276  -- Cool Hand Luke (1967)
, 10
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1221  -- Godfather: Part II, The (1974)
, 10
, 'hai bramli'
, 'Popular within the same genre community'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 48516  -- Departed, The (2006)
, 10
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1213  -- Goodfellas (1990)
, 10
, 'hai bramli'
, 'Both movies are award-winning'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1267  -- Manchurian Candidate, The (1962)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Narrative tension and payoff are comparable'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 912  -- Casablanca (1942)
, 9
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Narrative tension and payoff are comparable'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 58559  -- Dark Knight, The (2008)
, 9
, 'hai bramli'
, 'Popular within the same genre community'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 50  -- Usual Suspects, The (1995)
, 10
, 'hai bramli'
, 'Share narrative style or pacing'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 926  -- All About Eve (1950)
, 9
, 'hai bramli'
, 'Both movies are award-winning'
, 'Strong character development is a key appeal in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1245  -- Miller''s Crossing (1990)
, 9
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 527  -- Schindler''s List (1993)
, 10
, 'hai bramli'
, 'Share narrative style or pacing'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 3275  -- Boondock Saints, The (2000)
, 9
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'Strong character development is a key appeal in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1208  -- Apocalypse Now (1979)
, 9
, 'hai bramli'
, 'Both feature outstanding performances'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 933  -- To Catch a Thief (1955)
, 10
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Strong character development is a key appeal in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 2329  -- American History X (1998)
, 9
, 'hai bramli'
, 'Popular within the same genre community'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1233  -- Boot, Das (Boat, The) (1981)
, 9
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Their critical reception and audience overlap is strong'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1252  -- Chinatown (1974)
, 9
, 'hai bramli'
, 'Notable for screenplay and dialogue'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 10
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(318  -- Shawshank Redemption, The (1994)
, 1089  -- Reservoir Dogs (1992)
, 10
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 898  -- Philadelphia Story, The (1940)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'A great double feature for drama lovers'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 475  -- In the Name of the Father (1993)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1204  -- Lawrence of Arabia (1962)
, 9
, 'hai bramli'
, 'Both movies are award-winning'
, 'Widely discussed in film circles for similar reasons'
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1235  -- Harold and Maude (1971)
, 10
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 2959  -- Fight Club (1999)
, 9
, 'hai bramli'
, 'Both movies are award-winning'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1276  -- Cool Hand Luke (1967)
, 9
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'A great double feature for drama lovers'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1221  -- Godfather: Part II, The (1974)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 48516  -- Departed, The (2006)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1213  -- Goodfellas (1990)
, 9
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Actors deliver standout roles in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 930  -- Notorious (1946)
, 9
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 912  -- Casablanca (1942)
, 9
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 58559  -- Dark Knight, The (2008)
, 10
, 'hai bramli'
, 'Notable for screenplay and dialogue'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1197  -- Princess Bride, The (1987)
, 9
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1212  -- Third Man, The (1949)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 926  -- All About Eve (1950)
, 9
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1245  -- Miller''s Crossing (1990)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Their critical reception and audience overlap is strong'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 527  -- Schindler''s List (1993)
, 10
, 'hai bramli'
, 'Popular within the same genre community'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 3275  -- Boondock Saints, The (2000)
, 9
, 'hai bramli'
, 'Popular within the same genre community'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1208  -- Apocalypse Now (1979)
, 10
, 'hai bramli'
, 'Both movies are award-winning'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 933  -- To Catch a Thief (1955)
, 10
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'A great double feature for drama lovers'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 2329  -- American History X (1998)
, 9
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1233  -- Boot, Das (Boat, The) (1981)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1252  -- Chinatown (1974)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'A great double feature for drama lovers'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 10
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 475  -- In the Name of the Father (1993)
, 9
, 'hai bramli'
, 'Popular within the same genre community'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1204  -- Lawrence of Arabia (1962)
, 10
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'Both movies leave a lasting emotional impact'
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1235  -- Harold and Maude (1971)
, 9
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Narrative tension and payoff are comparable'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 2959  -- Fight Club (1999)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1276  -- Cool Hand Luke (1967)
, 10
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 750  -- Dr. Strangelove or: How I Learned to Stop Worrying and Love the Bomb (1964)
, 10
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1221  -- Godfather: Part II, The (1974)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Narrative tension and payoff are comparable'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 48516  -- Departed, The (2006)
, 9
, 'hai bramli'
, 'Notable for screenplay and dialogue'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1213  -- Goodfellas (1990)
, 9
, 'hai bramli'
, 'Both feature outstanding performances'
, 'Actors deliver standout roles in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 930  -- Notorious (1946)
, 9
, 'hai bramli'
, 'Share narrative style or pacing'
, 'Film students often analyze both for storytelling'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 912  -- Casablanca (1942)
, 9
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'Their critical reception and audience overlap is strong'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 58559  -- Dark Knight, The (2008)
, 9
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1197  -- Princess Bride, The (1987)
, 9
, 'hai bramli'
, 'Both movies are award-winning'
, 'Fans of one often appreciate the other for thematic depth'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 926  -- All About Eve (1950)
, 9
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'Strong character development is a key appeal in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1245  -- Miller''s Crossing (1990)
, 10
, 'hai bramli'
, 'Cult classics with loyal fanbases'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 527  -- Schindler''s List (1993)
, 10
, 'hai bramli'
, 'Notable for screenplay and dialogue'
, 'Actors deliver standout roles in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 3275  -- Boondock Saints, The (2000)
, 10
, 'hai bramli'
, 'Popular within the same genre community'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1208  -- Apocalypse Now (1979)
, 10
, 'hai bramli'
, 'Recommended for fans of classic storytelling'
, 'They showcase similar cinematographic techniques'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 933  -- To Catch a Thief (1955)
, 10
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'Narrative tension and payoff are comparable'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 2329  -- American History X (1998)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'Strong character development is a key appeal in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1233  -- Boot, Das (Boat, The) (1981)
, 9
, 'hai bramli'
, 'Critically acclaimed and share similar themes'
, 'A great double feature for drama lovers'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(898  -- Philadelphia Story, The (1940)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Widely discussed in film circles for similar reasons'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(475  -- In the Name of the Father (1993)
, 1204  -- Lawrence of Arabia (1962)
, 10
, 'hai bramli'
, 'Explore similar emotional journeys'
, 'A great double feature for drama lovers'
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(475  -- In the Name of the Father (1993)
, 1235  -- Harold and Maude (1971)
, 10
, 'hai bramli'
, 'Directed by influential filmmakers'
, 'Actors deliver standout roles in both'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(475  -- In the Name of the Father (1993)
, 2959  -- Fight Club (1999)
, 10
, 'hai bramli'
, 'Share narrative style or pacing'
, 'Both movies leave a lasting emotional impact'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 1104  -- Streetcar Named Desire, A (1951)
, 6
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 318  -- Shawshank Redemption, The (1994)
, 4
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 6
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 898  -- Philadelphia Story, The (1940)
, 4
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 475  -- In the Name of the Father (1993)
, 7
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 246  -- Hoop Dreams (1994)
, 3
, 'hai bramli'
, 'Very different eras or directors'
, NULL
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 1235  -- Harold and Maude (1971)
, 6
, 'hai bramli'
, 'Mismatch in theme, but both offer unique cinematic experiences'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 1276  -- Cool Hand Luke (1967)
, 3
, 'hai bramli'
, 'Randomly paired with minimal connection'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 750  -- Dr. Strangelove or: How I Learned to Stop Worrying and Love the Bomb (1964)
, 7
, 'hai bramli'
, 'Randomly paired with minimal connection'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 1221  -- Godfather: Part II, The (1974)
, 3
, 'hai bramli'
, 'Obscure mix of genres that could appeal to niche tastes'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 1213  -- Goodfellas (1990)
, 3
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 912  -- Casablanca (1942)
, 7
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 926  -- All About Eve (1950)
, 7
, 'hai bramli'
, 'Randomly paired with minimal connection'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 527  -- Schindler''s List (1993)
, 2
, 'hai bramli'
, 'Mismatch in theme, but both offer unique cinematic experiences'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 2329  -- American History X (1998)
, 7
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1499  -- Anaconda (1997)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 2
, 'hai bramli'
, 'Very different eras or directors'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 1104  -- Streetcar Named Desire, A (1951)
, 6
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 318  -- Shawshank Redemption, The (1994)
, 7
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 3
, 'hai bramli'
, 'Connection exists only in broad categorization'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 898  -- Philadelphia Story, The (1940)
, 4
, 'hai bramli'
, 'Low relevance, but might suit genre explorers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 475  -- In the Name of the Father (1993)
, 7
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 1204  -- Lawrence of Arabia (1962)
, 4
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 246  -- Hoop Dreams (1994)
, 3
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, NULL
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 1235  -- Harold and Maude (1971)
, 4
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 1276  -- Cool Hand Luke (1967)
, 4
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 750  -- Dr. Strangelove or: How I Learned to Stop Worrying and Love the Bomb (1964)
, 2
, 'hai bramli'
, 'Very different eras or directors'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 1221  -- Godfather: Part II, The (1974)
, 5
, 'hai bramli'
, 'Very different eras or directors'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 1213  -- Goodfellas (1990)
, 3
, 'hai bramli'
, 'Very different eras or directors'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 912  -- Casablanca (1942)
, 2
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 926  -- All About Eve (1950)
, 3
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 527  -- Schindler''s List (1993)
, 7
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 2329  -- American History X (1998)
, 6
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(1882  -- Godzilla (1998)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 4
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1104  -- Streetcar Named Desire, A (1951)
, 7
, 'hai bramli'
, 'Low relevance, but might suit genre explorers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 318  -- Shawshank Redemption, The (1994)
, 6
, 'hai bramli'
, 'Randomly paired with minimal connection'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 7
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 475  -- In the Name of the Father (1993)
, 4
, 'hai bramli'
, 'Randomly paired with minimal connection'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 246  -- Hoop Dreams (1994)
, 2
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, 'For those open to surprises in movie selection'
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1276  -- Cool Hand Luke (1967)
, 7
, 'hai bramli'
, 'Obscure mix of genres that could appeal to niche tastes'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 904  -- Rear Window (1954)
, 3
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1221  -- Godfather: Part II, The (1974)
, 7
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 48516  -- Departed, The (2006)
, 2
, 'hai bramli'
, 'Mismatch in theme, but both offer unique cinematic experiences'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1213  -- Goodfellas (1990)
, 6
, 'hai bramli'
, 'Low relevance, but might suit genre explorers'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1267  -- Manchurian Candidate, The (1962)
, 6
, 'hai bramli'
, 'Connection exists only in broad categorization'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 930  -- Notorious (1946)
, 3
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 912  -- Casablanca (1942)
, 4
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 50  -- Usual Suspects, The (1995)
, 3
, 'hai bramli'
, 'Randomly paired with minimal connection'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1212  -- Third Man, The (1949)
, 5
, 'hai bramli'
, 'Mismatch in theme, but both offer unique cinematic experiences'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 926  -- All About Eve (1950)
, 5
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1245  -- Miller''s Crossing (1990)
, 5
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 527  -- Schindler''s List (1993)
, 7
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 933  -- To Catch a Thief (1955)
, 7
, 'hai bramli'
, 'Mismatch in theme, but both offer unique cinematic experiences'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 2329  -- American History X (1998)
, 2
, 'hai bramli'
, 'Mismatch in theme, but both offer unique cinematic experiences'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1252  -- Chinatown (1974)
, 6
, 'hai bramli'
, 'Randomly paired with minimal connection'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 7
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(546  -- Super Mario Bros. (1993)
, 1089  -- Reservoir Dogs (1992)
, 2
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1104  -- Streetcar Named Desire, A (1951)
, 5
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 318  -- Shawshank Redemption, The (1994)
, 3
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 7
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 475  -- In the Name of the Father (1993)
, 5
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1204  -- Lawrence of Arabia (1962)
, 7
, 'hai bramli'
, 'Connection exists only in broad categorization'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 246  -- Hoop Dreams (1994)
, 2
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 2959  -- Fight Club (1999)
, 6
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1276  -- Cool Hand Luke (1967)
, 7
, 'hai bramli'
, 'Very different eras or directors'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 904  -- Rear Window (1954)
, 3
, 'hai bramli'
, 'Mismatch in theme, but both offer unique cinematic experiences'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1221  -- Godfather: Part II, The (1974)
, 3
, 'hai bramli'
, 'Randomly paired with minimal connection'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 48516  -- Departed, The (2006)
, 2
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1213  -- Goodfellas (1990)
, 5
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1267  -- Manchurian Candidate, The (1962)
, 5
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 930  -- Notorious (1946)
, 7
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 912  -- Casablanca (1942)
, 2
, 'hai bramli'
, 'Low relevance, but might suit genre explorers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 58559  -- Dark Knight, The (2008)
, 2
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 50  -- Usual Suspects, The (1995)
, 5
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1212  -- Third Man, The (1949)
, 4
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 926  -- All About Eve (1950)
, 2
, 'hai bramli'
, 'Unlikely pair, yet potentially interesting contrast'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1245  -- Miller''s Crossing (1990)
, 2
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 527  -- Schindler''s List (1993)
, 4
, 'hai bramli'
, 'Low relevance, but might suit genre explorers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 3275  -- Boondock Saints, The (2000)
, 3
, 'hai bramli'
, 'Randomly paired with minimal connection'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1208  -- Apocalypse Now (1979)
, 2
, 'hai bramli'
, 'Randomly paired with minimal connection'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 933  -- To Catch a Thief (1955)
, 4
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 2329  -- American History X (1998)
, 7
, 'hai bramli'
, 'Randomly paired with minimal connection'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1233  -- Boot, Das (Boat, The) (1981)
, 6
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1252  -- Chinatown (1974)
, 3
, 'hai bramli'
, 'Randomly paired with minimal connection'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1198  -- Raiders of the Lost Ark (Indiana Jones and the Raiders of the Lost Ark) (1981)
, 2
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, 'For those open to surprises in movie selection'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1193  -- One Flew Over the Cuckoo''s Nest (1975)
, 3
, 'hai bramli'
, 'Randomly paired with minimal connection'
, 'Might appeal to viewers who enjoy unusual juxtapositions'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2053  -- Honey, I Blew Up the Kid (1992)
, 1089  -- Reservoir Dogs (1992)
, 6
, 'hai bramli'
, 'Connection exists only in broad categorization'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 1104  -- Streetcar Named Desire, A (1951)
, 4
, 'hai bramli'
, 'Very different eras or directors'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 318  -- Shawshank Redemption, The (1994)
, 3
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 922  -- Sunset Blvd. (a.k.a. Sunset Boulevard) (1950)
, 2
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, 'Both challenge the viewer, in very different ways'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 475  -- In the Name of the Father (1993)
, 5
, 'hai bramli'
, 'Obscure mix of genres that could appeal to niche tastes'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 246  -- Hoop Dreams (1994)
, 7
, 'hai bramli'
, 'Obscure mix of genres that could appeal to niche tastes'
, 'Both challenge the viewer, in very different ways'
);


INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 1276  -- Cool Hand Luke (1967)
, 6
, 'hai bramli'
, 'Different tones and genres, but might intrigue curious viewers'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 904  -- Rear Window (1954)
, 6
, 'hai bramli'
, 'Weak thematic overlap, yet shared cult status'
, NULL
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 1221  -- Godfather: Part II, The (1974)
, 4
, 'hai bramli'
, 'Contrasting styles may challenge typical expectations'
, 'Could spark interesting discussion on genre contrast'
);

INSERT INTO imdb_ijs.movies_recommendations VALUES 
(2720  -- Inspector Gadget (1999)
, 48516  -- Departed, The (2006)
, 5
, 'hai bramli'
, 'Low relevance, but might suit genre explorers'
, 'Both challenge the viewer, in very different ways'
);